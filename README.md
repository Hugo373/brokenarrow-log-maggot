...
本地工具，只读《Broken Arrow》的 GameLogs：实时识别当前对局与名单，查询公开统计并计算玩家表现分（仪表盘 0~100，越高越强）；同时用本地数据库追踪你遇到过的玩家——相遇战绩、改名历史、封禁提醒。一切呈现在浏览器仪表盘：局前速览、局后复盘与诊断。
...
- **对局历史**：「历史」页签列出本地记录的全部对局（时间、地图、FID、胜负、人数、名单），点击行展开玩家明细
