...
- **API governance**: per-endpoint cache TTLs, fresh/stale distinction, tiered circuit breaker, throttled retries, persisted rolling 24h quota, human-verification page detection, stale-cache fallback, automatic retry aligned to breaker windows; falls back to a direct connection when the system proxy fails
